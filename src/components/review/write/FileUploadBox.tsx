...
export function FileUploadBox({ value, onFileChange }: FileUploadBoxProps) {
  return (
    <label
      htmlFor="file-upload"
      className="border-border hover:bg-bg-sub relative cursor-pointer rounded-lg border-2 border-dashed text-center transition"
    >
      {value ? (
        <Image
          src={value}
