...
export const updateReviewPost = async (
  postId: number,
  data: ReviewPostWrite & { remainImageUrls?: string[] }
): Promise<boolean> => {
  try {
    const formData = new FormData();
    ...
    // 유지할 기존 이미지 URL (옵션)
    if (data.remainImageUrls && data.remainImageUrls.length > 0) {
      data.remainImageUrls.forEach((url) => {
        formData.append("remainImageUrls", url);
      });
    }
    ...
    if (!res.ok || json.resultCode !== "OK") {
      let fallbackMsg = "리뷰 수정 중 오류가 발생했습니다.";
