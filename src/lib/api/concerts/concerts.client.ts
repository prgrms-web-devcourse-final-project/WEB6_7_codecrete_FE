...
/**
 * @description 공연 ID로 유사한 공연 목록 가져오기
 * @param {string} concertId - 공연 ID
 * @returns {Promise<ConcertWithTicket[] | null>} - 유사한 공연 목록 또는 null
 */
export const getSimilarConcerts = async ({
  ...
  try {
    const res = await ClientApi(`/api/v1/concerts/similarConcerts/${concertId}`, {
      method: "GET",
    });
    if (!res.ok) {
      console.error("API Error:", res.status, res.statusText);
