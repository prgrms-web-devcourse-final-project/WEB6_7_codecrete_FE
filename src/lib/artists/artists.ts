...
export async function getArtistDetail(id: number): Promise<ArtistDetail> {
  try {
    // fetch (네트워크 단계)
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/v1/artists/${id}`, {
      cache: "no-store",
    });
    ...
    try {
      json = (await res.json()) as ArtistDetailResponse;
    } catch {
      // JSON 파싱 실패 (빈 응답, HTML 응답 등)
      throw new Error("서버 응답을 처리할 수 없습니다.");
    }
    ...
    // HTTP / 비즈니스 에러 분기
    if (!res.ok || json.resultCode !== "OK") {
      throw new Error(json.msg ?? "아티스트 정보를 불러오는 중 오류가 발생했습니다.");
    }
